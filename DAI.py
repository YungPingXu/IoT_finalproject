...
mac_addr = 'XYP2' #+ str( random.randint(100,999 ) )  # put here for easy to modify :-)
...
DAN.profile['dm_name'] = '0716208'   # you can change this but should also add the DM in server
DAN.profile['df_list'] = ['server-IDF', 'server-ODF']   # Check IoTtalk to see what IDF/ODF the DM has
DAN.profile['d_name'] = "XYP" #+ str( random.randint(100,999 ) ) +"_"+ DAN.profile['dm_name'] # None
...
while True:
	try:
		...
		value1=DAN.pull('server-ODF')
		if value1 != None:
			print (value1[0],  value1[1], value1[2], value1[3], value1[4])
		#Push data to a device feature called "Dummy_Sensor"
		#value2=random.uniform(1, 10)    ## original Dummy_Device example
		if gotInput:
			# we have data in theInput
			value2=theInput.split(" ")
			if allDead: break
			gotInput=False   # so that you can input again
			DAN.push ('server-IDF', value2[0],  value2[1], value2[2], value2[3], value2[4])  #  試這:  DAN.push('Dummy_Sensor', theInput)
